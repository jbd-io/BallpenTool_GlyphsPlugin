...
def trim_ends(points, trim_length=2.0):
    ...
    et les doublons de points. Le trimming agit sur les deux premiers et deux derniers segments.
    """
    if len(points) < 3:
        return points[:]
    ...
    # Trim environ 1/20 de la largeur du trait
    trim = trim_length * 1.5
    ...
    if distance(trimmed[0], trimmed[1]) < 0.1:
        trimmed = trimmed[1:]
    if distance(trimmed[-1], trimmed[-2]) < 0.1:
        trimmed = trimmed[:-1]
...
def clamp_tangents(beziers):
    ...
    Corrige aussi les surlongueurs sur les 2e et avant-dernier segments.
    """
    if not beziers:
        return beziers
